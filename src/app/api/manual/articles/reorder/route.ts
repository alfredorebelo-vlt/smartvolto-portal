...
// PATCH /api/manual/articles/reorder — salva nova ordem dos artigos
// Body: { ids: string[] }  — array de IDs na nova ordem desejada
export async function PATCH(req: NextRequest) {
  ...
  await prisma.$transaction(
    body.ids.map((id, idx) =>
      prisma.manualArticle.update({ where: { id }, data: { order: idx } })
    )
  );
